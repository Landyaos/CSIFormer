...
class CSIFormerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        return self.csi_ls[idx], self.csi_pre[idx, 1:2, :, :, :, :, :], self.csi_label[idx]
...
def train_model(model, dataloader_train, dataloader_val, criterion, optimizer, scheduler, epochs, device, checkpoint_dir='./checkpoints'):
    ...
    best_loss = float('inf')
    best_model_counter = 1  # 用于标记最佳模型的版本号
    start_epoch = 0
    model.to(device)
    
    # 固定保存最新checkpoint的文件名，不附版本号（用于断点续训）
    ...
    if os.path.isfile(latest_path):
        ...
        best_model_counter = checkpoint.get('best_model_counter', best_model_counter)
        print(f"[INFO] Resumed epoch {start_epoch}, best_loss={best_loss:.6f}, best_model_counter={best_model_counter}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # 学习率调度器步进（例如 ReduceLROnPlateau 需要传入指标）
        ...
        with torch.no_grad():
            for batch_idx, (csi_ls_val, pre_csi_val, csi_label) in enumerate(dataloader_val):
                ...
                loss_val = criterion(csi_dec, csi_label)
        ...
        checkpoint_dict = {
            ...
            'best_loss': best_loss,
            'best_model_counter': best_model_counter,
        }
        
        # 1) 保存最新 checkpoint（固定文件名，用于断点续训）
        torch.save(checkpoint_dict, latest_path)

        # 2) 如果当前验证集 loss 优于之前的最佳值，则保存为新的 best 模型，并附上版本号
        if val_loss < best_loss:
            best_loss = val_loss
            best_model_path = os.path.join(checkpoint_dir, model.__class__.__name__ + f'_v{best_model_counter}_best.pth')
            torch.save(checkpoint_dict, best_model_path)
            print(f"[INFO] Best model saved at epoch {epoch + 1}, version v{best_model_counter}, val_loss={val_loss:.4f}")
