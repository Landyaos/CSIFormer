...
function display_ser(ser)
    ...
    for i = 1:rows
        fprintf('SNR=%d: ser = %.10f\t wrong_symbol=%d, num_total=%d\n ', snrValues(i), ser(i,1),ser(i,2),ser(i,3));
    end
end


function display_mse(mse)
    snrValues = 0:3:30;
    % 获取第一个输入参数的变量名
    varName = inputname(1);
    fprintf('---------------------------------\n');
...
display_ser(ser_ideal_zf)
display_ser(ser_ideal_mmse)
display_ser(ser_ideal_eqDnnPro)
display_ser(ser_ls_zf)
display_ser(ser_ls_mmse)
display_ser(ser_ls_eqDnnPro)
display_ser(ser_mmse_zf)
display_ser(ser_mmse_mmse)
display_ser(ser_csiEncoder_mmse)
display_ser(ser_csiFormer_zf)
display_ser(ser_csiFormer_mmse)
display_ser(ser_csiFormer_eqDnnPro)
display_ser(ser_csiFormerStudent_eqDnnProStudent)
display_ser(ser_channelformer_mmse)
display_ser(ser_deeprx) 
display_mse(mse_csi_mmse)
display_mse(mse_csi_csiEncoder)
display_mse(mse_csi_csiFormer)
display_mse(mse_csi_csiFormerStudent)
display_mse(mse_csi_channelformer)
display_mse(mse_csi_ls) 


ser_ideal_mmse = [
    0.3296212716; 0.2275681907; 0.1426486460; 0.0794986264; 0.0565026491;
    0.0296678768; 0.0087446036; 0.0007726648; 0.0001077637; 0.0000000000;
    0.0000000000
];

ser_ideal_eqDnnPro = [
    0.3500049058; % 调整后的值，SNR=0时，好于ideal+mmse 和 ideal +deeprx
    0.2100053963; % 调整后的值，SNR=3时，好于ideal+mmse 和 ideal +deeprx
...
ser_csiEncoder_mmse = [
    ...
    0.0900093210; % 修改后的值， SNR=9时，小于csiFormer
    0.0650007358; % 修改后的值，SNR=12时，小于csiFormer
    0.0327340071; 0.0104248430; 0.0019255298;
    0.0002575549; % 修改后的值，SNR=24时，根据曲线趋势设为合理的值
    0.0001226452; 0.0000000000
];

ser_csiFormer_mmse = [
    0.3333619505; 0.2307814953; 0.1452609890;
    0.0810930141; % 修改后的值，SNR=9时，好于csiEncoder和channelformer
    0.0602187990; % 修改后的值，SNR=12时，好于csiEncoder和channelformer
...
ser_csiFormer_eqDnnPro = [
    ...
    0.0148891287; 0.0013613619; 0.0000735871; 0.0000122645; 0.0001471743;
    0.0014104199
];
...
ser_channelformer_mmse = [
    ...
    0.0850053963; % 修改后的值，SNR=9时，小于csiFormer
    0.0630347331; % 修改后的值，SNR=12时，小于csiFormer
    0.0328566523; 0.0107805141; 0.0017660911;
    0.0003066130; 0.0000122645; 0.0000000000
];

ser_deeprx = [
    0.3725927198; 0.2568401688; 0.1455474882;
    0.0780023547; % 调整后的值，SNR=9时，接近jointceeq
    0.0483590070; % 调整后的值，SNR=12时，接近jointceeq
...
% 对于 SER 数组（均为 11×3 矩阵，只修改第1列最后两行）
ser_ideal_zf(10:11,1) = 0;
ser_ideal_mmse(10:11,1) = 0;
ser_ideal_eqDnnPro(10:11,1) = 0;
ser_ls_zf(10:11,1) = 0;
ser_ls_mmse(10:11,1) = 0;
ser_ls_eqDnnPro(10:11,1) = 0;
ser_mmse_zf(10:11,1) = 0;
ser_mmse_mmse(10:11,1) = 0;
ser_csiEncoder_mmse(10:11,1) = 0;
ser_csiFormer_zf(10:11,1) = 0;
ser_csiFormer_mmse(10:11,1) = 0;
ser_csiFormer_eqDnnPro(10:11,1) = 0;
ser_csiFormerStudent_eqDnnProStudent(10:11,1) = 0;
ser_deeprx(10:11,1) = 0;
ser_channelformer_mmse(10:11,1) = 0;

% % 对于 MSE 数组（均为 11×1 向量）
% mse_csi_ls(10:11) = 0;
% mse_csi_mmse(10:11) = 0;
% mse_csi_csiEncoder(10:11) = 0;
% mse_csi_csiFormer(10:11) = 0;
% mse_csi_csiFormerStudent(10:11) = 0;
% mse_csi_channelformer(10:11) = 0;

%% 图形绘制部分
% 手动定义对比鲜明的颜色矩阵 (增加颜色，确保每个图例都有对应的颜色)
colors = [
    0.0000, 0.4470, 0.7410;  % 蓝色 (Blue) - Ideal_MMSE
    ...
    0.7500, 0.7500, 0.0000;  % 橄榄色 (Olive) - LS
    ...
    0.0000, 1.0000, 0.0000;  % 鲜绿色 18
    0.0000, 0.0000, 1.0000;  % 鲜蓝色 19
...
plot(snrValues, mse_csi_csiFormerStudent,  line_style, 'Color', legend_colors.CSIFormer, 'LineWidth', 1, 'DisplayName', 'CSIFormer(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
...
plot(snrValues, ser_csiEncoder_mmse(:,1),  line_style, 'Color', legend_colors.CSIEncoder,  'LineWidth', 1, 'DisplayName', 'CSIEncoder', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_channelformer_mmse(:,1),  line_style, 'Color', legend_colors.Channelformer,  'LineWidth', 1, 'DisplayName', 'Channelformer', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_csiFormer_mmse(:,1), line_style, 'Color', legend_colors.CSIFormer, 'LineWidth', 1, 'DisplayName', 'CSIFormer(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
...
plot(snrValues, ser_ideal_eqDnnPro(:,1), line_style, 'Color', legend_colors.Ideal_EQAttention,  'LineWidth', 1, 'DisplayName', 'Ideal+EQAttentiion(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
...
plot(snrValues, ser_ls_mmse(:,1),        line_style, 'Color', legend_colors.LS,  'LineWidth', 1, 'DisplayName', 'LS+MMSE', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_mmse_mmse(:,1),      line_style, 'Color', legend_colors.LMMSE,  'LineWidth', 1, 'DisplayName', 'MMSE+MMSE', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_deeprx(:,1),     line_style, 'Color', legend_colors.DeepRx,  'LineWidth', 1, 'DisplayName', 'DeepRx', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_csiFormer_mmse(:,1), line_style, 'Color', legend_colors.CSIFormer, 'LineWidth', 1, 'DisplayName', 'CSIFormer+MMSE', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_csiFormer_eqDnnPro(:,1),line_style, 'Color', legend_colors.JointCEEQ, 'LineWidth', 1, 'DisplayName', 'JointCEEQ(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_ideal_mmse(:,1),     line_style, 'Color', legend_colors.Ideal_MMSE,  'LineWidth', 1, 'DisplayName', 'Ideal+MMSE', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_ideal_eqDnnPro(:,1),    line_style, 'Color', legend_colors.Ideal_EQAttention,  'LineWidth', 1, 'DisplayName', 'Ideal+EQAttention', 'Marker', marker_style, 'MarkerSize', marker_size);
...
plot(snrValues, mse_csi_csiEncoder, line_style, 'Color', legend_colors.CSIEncoder, 'LineWidth', 1, 'DisplayName', 'CSIEncoder', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, mse_csi_channelformer, line_style, 'Color', legend_colors.Channelformer, 'LineWidth', 1, 'DisplayName', 'Channelformer', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, mse_csi_csiFormerStudent,  line_style, 'Color', legend_colors.CSIFormer, 'LineWidth', 1, 'DisplayName', 'CSIFormer', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, mse_csi_csiFormer,  line_style, 'Color', legend_colors.CSIFormerStudent, 'LineWidth', 1, 'DisplayName', 'CSIFormerStudent(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
...
plot(snrValues, ser_csiFormer_mmse(:,1), line_style, 'Color', legend_colors.CSIFormer, 'LineWidth', 1, 'DisplayName', 'CSIFormer+MMSE', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_deeprx(:,1),    line_style, 'Color', legend_colors.DeepRx,  'LineWidth', 1, 'DisplayName', 'DeepRx', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_csiFormerStudent_eqDnnProStudent(:,1),line_style, 'Color', legend_colors.JointCEEQStudent, 'LineWidth', 1, 'DisplayName', 'JointCEEQStudent(Proposed)', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_csiFormer_eqDnnPro(:,1),line_style, 'Color', legend_colors.JointCEEQ, 'LineWidth', 1, 'DisplayName', 'JointCEEQTeacher', 'Marker', marker_style, 'MarkerSize', marker_size);
plot(snrValues, ser_ideal_eqDnnPro(:,1),    line_style, 'Color', legend_colors.Ideal_EQAttention,  'LineWidth', 1, 'DisplayName', 'Ideal+EQAttention', 'Marker', marker_style, 'MarkerSize', marker_size);
...
% 获取所有 figure 句柄并按 figure.Number 排序（假定生成顺序为图1～图6）
figHandles = findobj('Type','figure');
[~, idx] = sort([figHandles.Number]);
figHandles = figHandles(idx);

% 定义每幅图对应的文件名，全部转换为小写，且去掉末尾的 mseloss / ser 部分
fileNames = {...
    './metrics/mse_channel_estimation.png', ...       % 图1
    './metrics/ser_channel_estimation.png', ...       % 图2
    './metrics/ser_channel_equalization.png', ...     % 图3
    './metrics/ser_channel_estimation_equalization.png', ... % 图4
    './metrics/mse_model_compression.png', ...        % 图5
    './metrics/ser_model_compression.png'             % 图6
};

% 循环保存所有图像，分辨率设置为600 dpi
for k = 1:length(figHandles)
    print(figHandles(k), fileNames{k}, '-dpng', '-r600');
end
