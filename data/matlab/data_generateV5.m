...
%% 数据集采集
numFrame = 2;
datasetPath = {'../raw/ceTrainData.mat','../raw/ceValData.mat'};
...
for datasetIdx = 1:length(datasetPath)
    ...
    txPilotData = zeros(datasetCapacity, numValidSubc, numSym, numTx, 2);
    rxPilotData = zeros(datasetCapacity, numValidSubc, numSym, numRx, 2);
    ...
    for snrIdx = 1:length(snrValues)
        ...
        for frame = -1:snrDatasetSize
            ...
            % 发送信号（实部和虚部分离）
            txPilotData(dataIdx,:,:,:,1) = real(originSignal(validSubcIndices, :, :));
            txPilotData(dataIdx,:,:,:,2) = imag(originSignal(validSubcIndices, :, :));
            % 接收信号（实部和虚部分离）
            rxPilotData(dataIdx,:,:,:,1) = real(finalSignal(validSubcIndices, :, :));
            rxPilotData(dataIdx,:,:,:,2) = imag(finalSignal(validSubcIndices, :, :));
