...
% 系统参数配置
numSubc = 256;                                            % FFT 长度
numGuardBands = [0;0];                                    % 左右保护带
numPilot = numSubc/4-1;                                   % 每根天线的导频子载波
numTx = 2;                                                % 发射天线数量
numRx = 2;                                                % 接收天线数量
numSym = 14;                                              % 每帧 OFDM 符号数
numStream = 2;                                            % 数据流个数
cpLength = numSubc/2;                                     % 循环前缀长度
...
% 导频子载波配置
pilotIndicesAnt1 = (1:4:numSubc)';                        % 发射天线1的导频子载波位置
pilotIndicesAnt2 = (2:4:numSubc)';                        % 发射天线2的导频子载波位置
...
pilotIndicesAnt1(end) = numSubc;
pilotIndicesAnt2(end) = numSubc-1;
...
for idx = 1:1:length(snrValues)
    ...
    for frame = 1:1:30
        ...
        noiseVar = noiseVar*numSubc;    % 子载波噪声功率
        hMMSE = MMSE_ChannelEst(rxPilotSignal, pilotSignal, dataIndices, pilotIndices, t_rms, delta_f, power_r, noiseVar);
...
plot(snrValues, serMMSEMMSE(:, 1), '-d', 'LineWidth', 1.5, 'DisplayName', 'LS MMSE');
...
function [H_est] = MMSE_ChannelEst(rxPilotSignal, refPilotSignal, dataIndices, pilotIndices, t_rms, delta_f, power_r, noiseVar)
    ...
    for tx = 1:numTx
        for rx = 1:numRx
            for sym = 1:numSym
                ...
                H_est(:,sym,tx,rx) = interp1(pilotIndices(:,sym,tx), H_MMSE, dataIndices, 'linear', 'extrap');
            end 
        end
    end
end
