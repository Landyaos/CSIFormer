...
snr = 10;
ofdmNullIdx = [1:numGuardBands(1),(numSubcarriers-numGuardBands(2)+1):numSubcarriers];
...
% 通过信道模型获取接收信号和路径增益
[rxSignal, pathGains] = mimoChannel(txSignal); % pathGains: [总样本数, N_path, numTransmitAntennas, numReceiveAntennas]
...
h = ofdmChannelResponse(pathGains, pathFilters, numSubcarriers, cpLength, dataSymbolIndices, toffset); % Nsc x Nsym x Nt x Nr
hReshaped = reshape(h,[],numTransmitAntennas,numReceiveAntennas);

% 噪声
[rxSignal, noiseVar] = awgn(rxSignal, snr, "measured");
...
equalizedSymbols = ofdmEqualize(rxDataSymbols,hReshaped, noiseVar);
