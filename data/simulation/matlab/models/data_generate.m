...
txDataSignalData = zeros(batchSize, numDataSubc, numSym, numTx, 2);
txPilotSignalData = zeros(batchSize, numPilotDataSubc, numSym, numTx, 2);
rxDataSignalData = zeros(batchSize, numDataSubc, numSym, numRx, 2);
rxPilotSignalData = zeros(batchSize, numPilotDataSubc, numSym, numRx, 2);
...
for i = 1:batchSize
    ...
    finalSignal(dataIndices, :, :) = rxDataSymbols(:,:,:);
    for rx = 1:numRx
        for tx = 1:numTx
            for sym = 1:numSym
            finalSignal(pilotIndices(:,sym,tx),sym,rx) = rxPilotSymbols(:,sym,tx,rx);
            end
        end
    end
    ...
    rxSignalData(i,:,:,:,1) = real(finalSignal(validSubcIndices, :,:));
    rxSignalData(i,:,:,:,2) = imag(finalSignal(validSubcIndices, :,:));
    
    % 发送数据信号（实部和虚部分离）
    txDataSignalData(i,:,:,:,1) = real(dataSignal); % 实部
    txDataSignalData(i,:,:,:,2) = imag(dataSignal); % 虚部
    
    % 发送导频信号（实部和虚部分离）
    txPilotData = originSignal(pilotDataIndices,:,:);

    txPilotSignalData(i,:,:,:,1) = real(txPilotData); % 实部
    txPilotSignalData(i,:,:,:,2) = imag(txPilotData); % 虚部
    
    % 接收数据符号（实部和虚部分离）
    rxDataSignalData(i,:,:,:,1) = real(rxDataSymbols); % 实部
    rxDataSignalData(i,:,:,:,2) = imag(rxDataSymbols); % 虚部
    
    rxPilotData = finalSignal(pilotDataIndices,:,:);

    % 接收导频符号（实部和虚部分离）
    rxPilotSignalData(i,:,:,:,1) = real(rxPilotData); % 实部
    rxPilotSignalData(i,:,:,:,2) = imag(rxPilotData); % 虚部
