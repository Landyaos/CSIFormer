...
H_inv = zeros(size(rxSignal,1), numTransmitAntennas, numReceiveAntennas);
rxEqualizedSignal = zeros(size(rxSignal));

for i = 1:size(rxSignal,1)
    H_inv(i, :, :) = squeeze(pathGains(i,1,:,:)) \ eye(numReceiveAntennas);
    rxEqualizedSignal(i, : ,:) = rxSignal(i, :, :) * squeeze(H_inv(i, :, :));
end
...
% OFDM 解调
[rxDataSymbols, rxPilotSymbols] = ofdmDemod(rxEqualizedSignal);
disp(rxPilotSymbols(:,1,1,1))
